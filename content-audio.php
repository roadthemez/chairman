...
<article id="post-<?php the_ID(); ?>" <?php post_class($chairman_postclass); ?>>
	...
	<?php 
		if ( is_single() ) { ?>
			...
			<div class="player"><?php echo do_shortcode(get_post_meta( $post->ID, '_chairman_meta_value_key', true )); ?></div>
		<?php }
	?>
	<?php if ( !is_single() ) { ?>
		...
		<div class="post-info"> 
			...
			<?php else : ?>
				<div class="entry-summary">
					<div class="player"><?php echo do_shortcode(get_post_meta( $post->ID, '_chairman_meta_value_key', true )); ?></div>
					<?php the_excerpt(); ?>
					<a class="readmore button" href="<?php the_permalink(); ?>"><?php if(isset($chairman_opt['readmore_text']) && $chairman_opt['readmore_text']!=''){ echo esc_html($chairman_opt['readmore_text']); } else { esc_html_e('Read more', 'chairman');}  ?></a>
			...
			<?php if ( is_single() ) : ?>
				...
				<div class="author-info">
					...
					<div class="author-description">
						<h2><?php esc_html_e( 'About the Author:', 'chairman'); printf( '<a href="'.esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ).'" rel="author">%s</a>' , get_the_author()); ?></h2>
